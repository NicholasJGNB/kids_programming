...
function makeIcon(size) {
  ...
  const roundRect = (x0, y0, w, h, rad, r, g, b, a = 255) => {
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
      // 圆角裁剪
      let cx = x, cy = y;
      let dx = Math.max(rad - x, x - (w - 1 - rad), 0);
      let dy = Math.max(rad - y, y - (h - 1 - rad), 0);
  ...
  // 机器人天线
  rect(Math.round(S*0.49), Math.round(S*0.16), Math.round(S*0.02), Math.round(S*0.08), 255,255,255);
  disc(Math.round(S*0.50), Math.round(S*0.15), Math.round(S*0.03), 0xff, 0xd8, 0x6b);
  // 机器人头（白色圆角方块）
  const hx = Math.round(S*0.24), hy = Math.round(S*0.26), hw = Math.round(S*0.52), hh = Math.round(S*0.44);
  roundRect(hx, hy, hw, hh, Math.round(S*0.10), 255, 255, 255);
  // 眼睛（蓝色圆点）
  ...
  // 嘴巴（橙色小横条）
  rect(Math.round(S*0.42), Math.round(S*0.58), Math.round(S*0.16), Math.round(S*0.03), 0xff, 0x9f, 0x43);
